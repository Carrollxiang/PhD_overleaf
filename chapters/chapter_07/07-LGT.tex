...
理解规范场论的非平衡动力学是现代高能物理学的核心挑战之一。在真实的物理世界中，从重离子碰撞到轻子与强子对撞，许多关键的物理过程都发生在远离平衡态的强相互作用演化之中。然而，对这类极度非平衡的动态过程进行严格的从头计算（\textit{ab initio}）极其困难。尽管格点规范场论为理解能谱和相图等平衡态性质提供了可靠的非微扰框架，但在处理实时演化动力学时，经典计算资源的需求呈指数级增长。在实验方面，传统的大型粒子对撞机能够精确测量渐近末态的产物，却难以对超快瞬态的中间散射过程实施可控的干预与直接读出。
...
\caption{\textbf{里德堡原子格点规范场论量子模拟器以及物理量映射关系。} }
  \label{ch7:fig:setup_and_LGT_mapping}
\end{figure}
\todo{这里需要重组图\ref{ch7:fig:setup_and_LGT_mapping}的内容，保留a-e}
...
\todo{(此处插入图 \ref{ch7:fig:confinement_and_deconfinement_phases}：对应原 `Fig2\_Confinement\_deconfinement\_phases.png`，展示不同参数下电场的时空演化及体电通量的稳态偏置)}
...
\todo{(此处插入图 \ref{ch7:fig:Quench_dynamics}：对应原 `Fig3\_Quench\_dynamics.png`，展示双参数淬火序列、弦破碎与光锥传播动力学)}
...
\todo{(此处插入图 \ref{ch7:fig:Meson_dynamics}：对应原 `Fig4\_Meson\_dynamics.png`，展示强禁闭与解禁闭下的电荷密度和动力学电荷演化)}
...
\todo{(此处可选择性插入图 \ref{ch7:fig:string-tension}：对应原补充材料 `EDFigure\_String\_tension.png`，展示不同弦张力下的演化与平均电场)}
...
\end{figure}
\todo{(此处插入图 \ref{ch7:fig:tune-masses}：对应原 EDFigure\_masses.png，展示不同费米子质量下的散射与弦张力淬火动力学)}
...
\todo{(此处插入图 \ref{ch7:fig:Freeze-frame-dynamics}：对应原 `Fig5\_Quantum\_freeze-out\_dynamics.png`，展示冻结协议、电荷/密度演化及纠缠熵对比)}
