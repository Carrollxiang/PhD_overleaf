...
\subsection{碰撞阻塞效应 (Collisional Blockade)}
...
\subsection{关键物理量的标度律 (Scaling Laws)}
里德堡原子的许多物理属性随有效主量子数 $n^*$ 呈现显著的幂律依赖关系。下表总结了对量子模拟至关重要的几个标度律 27：

\begin{table}[]
\begin{tabular}{l|l|l|l}
\hline
物理属性    & 符号                  & 标度关系                 & 物理机制与实验意义                                                      \\ \hline
轨道半径    & $\langle r \rangle$ & $\propto (n^*)^2$    & 电子轨道巨大。$n=100$ 时半径约为 $1 \, \mu\text{m}$，使得原子极易被极化。             \\
结合能     & $E_n$               & $\propto (n^*)^{-2}$ & 相邻能级间隔 $\Delta E \propto (n^*)^{-3}$。高 $n$ 态能级非常密集，需使用窄线宽激光激发。 \\
辐射寿命 & $\tau$ & $\propto (n^*)^3$ & 低 $l$ 态寿命受限于与基态的重叠积分（随 $n$ 减小）。$n \sim 60$ 时寿命达 $100 \, \mu\text{s}$，加上黑体辐射诱导跃迁后仍很长，允许进行复杂的量子操作。 \\
极化率     & $\alpha$            & $\propto (n^*)^7$    & 极化率极高，对外部电场极其敏感。微小的杂散电场都会导致巨大的能级移动（DC Stark Shift），需精密电场补偿。    \\
范德瓦尔斯系数 & $C_6$               & $\propto (n^*)^{11}$ & 相互作用强度随 $n$ 剧烈增强。这是实现快速两比特门和强关联物理模拟的关键。                       
\end{tabular}
\end{table}

\subsection{里德堡原子在电场中的斯塔克效应}
...
\section{里德堡相互作用与多体哈密顿量}
里德堡原子间极强的相互作用是实现量子纠缠和模拟多体物理的资源。
\subsection{多极展开与相互作用形式}
