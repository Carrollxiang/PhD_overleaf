...
由于电子自旋 $\mathbf{S}$ ($S=1/2$) 与轨道角动量 $\mathbf{L}$ ($L=1$ 对于 P 态) 的自旋-轨道耦合作用 ($H_{SO} \propto \mathbf{L}\cdot\mathbf{S}$)，$5P$ 态分裂为 $5P_{1/2}$ 和 $5P_{3/2}$ 两个精细能级 [5]。
\begin{itemize}
...
进一步考虑原子核自旋 $\mathbf{I}$ (对于 $^{87}\text{Rb}$，$I=3/2$) 与电子总角动量 $\mathbf{J}$ 的磁偶极相互作用，能级发生超精细分裂 (Hyperfine Splitting)，分裂出的子能级由总角动量量子数 $F = |J-I|, \dots, J+I$ 标记 [7]。
\begin{itemize}
    \item \textbf{基态 $5S_{1/2}$} 分裂为 $F=1$ 和 $F=2$ 两个能级，其频率间隔定义了铷原子的钟跃迁频率 $\Delta \nu_{\text{hfs}} \approx 6.834\,\text{GHz}$ [8]。
...
在弱场近似下 (Zeeman regime)，能级移动由朗德 g 因子 $g_F$ 决定 [9]：
\begin{equation}
\Delta E_{|F, m_F\rangle} \approx \mu_B g_F m_F B
\end{equation}
对于基态 $^{87}\text{Rb}$，$F=2$ 态的 $g_F \approx 1/2$，而 $F=1$ 态的 $g_F \approx -1/2$。
这意味着在磁场极小值处（如 MOT 中心），$|F=2, m_F>0\rangle$ 的弱场寻找态 (Low-field seekers) 可以被磁场俘获，而 $|F=1, m_F=0\rangle$ 对一阶磁场不敏感，常被选为对磁噪声免疫的量子存储态（钟态） [8]。
...
当使用蓝失谐激光 ($\Delta > 0$) 照射原子对时，激光频率与相互作用势能曲线中的排斥性分支共振 [10]。这种激发使得两个原子在靠近过程中经历排斥力，从而有效地“屏蔽”了短距离的非弹性碰撞通道。或者，通过精细调节激光频率至特定的分子共振处，可以诱导一种受控的能量转移过程：当两个原子发生碰撞时，仅赋予其中一个原子足够的动能使其逃逸，而另一个原子仍被保留在阱中。这种机制可以将单原子装载率提升至 80\% 以上。
...
空间光调制器 (SLM) 是一种能够对光束波前进行像素级相位调制的器件。通过在 SLM 平面 (傅里叶面) 加载特定的相位全息图 $\phi(x,y)$，经由透镜的傅里叶变换作用，可以在焦平面 (原子平面) 生成任意几何构型的光阱阵列 [14]。
...
\end{itemize}
为了提高阵列中各个光镊强度的均匀性，我们采用了加权 GS 算法 (Weighted GS, WGS) [16, 17]。该算法引入了一个反馈增益，根据每次迭代后焦平面各点强度的偏差，动态调整目标振幅权重，从而不仅能生成任意几何形状，还能补偿光学系统的像差和非均匀性。

\subsection{动态原子输运：AOD 偏转原理}
声光偏转器 (AOD) 提供了对光镊位置的快速、动态控制，是实现原子重排 (Atom Rearrangement) 的核心器件。AOD 基于声光效应，当射频信号 (RF) 加载到晶体上时，会在晶体内形成移动的折射率光栅。根据布拉格衍射方程，一级衍射光的偏转角度 $\theta$ 与 RF 频率 $f$ 成正比 [18]：
\begin{equation}
\Delta \theta = \frac{\lambda}{v_s} \Delta f
\end{equation}
其中 $\lambda$ 为光波长，$v_s$ 为晶体中的声速。通过改变驱动频率 $f$，可以精确控制光镊在焦平面的位置；通过改变 RF 功率，可以控制光镊的深度。利用多频 RF 信号驱动 (Multi-tone driving)，AOD 可以同时产生多个移动光镊，实现并行化的原子输运与纠错 [19, 20]。
...
R_b = \left( \frac{|C_6|}{\hbar\Omega} \right)^{1/6}
\end{equation}
在阻塞半径内，原子系综的激发表现出集体行为，形成了所谓的“超原子” (Superatom)，其基态到单激发态的有效拉比频率增强为 $\sqrt{N}\Omega$ [25]。这一效应被广泛用于实现高保真度的两比特纠缠门和量子模拟。
