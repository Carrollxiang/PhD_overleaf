...
\section{引言}
在第五章中，我们通过高保真度的初态制备，成功在里德堡原子阵列中观测到了宏观单体可观测量（如里德堡布居数与畴壁密度）的周期性塌缩与复苏。然而，一个极其关键的物理问题随之浮现：系统在宏观密度层面上免于全局热化，是否必然意味着局域的微观量子信息也得到了保护或发生了回流？事实上，在强相互作用的多体系统中，即使宏观波函数或密度分布展现出完美的复苏，局域编码的量子信息也完全有可能在极其复杂的受限希尔伯特空间内部发生深度的加扰（Scrambling）与耗散。宏观的单体密度探针根本无力穿透这一多体纠缠演化的“黑盒”。

为了在逻辑上严密地证明“宏观密度复苏”绝不等于“微观信息回流”，我们可以考察一个启发性的反例——完全无约束的疤痕“玩具模型”（Toy model，详见本章 6.4 节讨论）\cite{choi2019emergent}。理论与数值分析确凿地表明，在该玩具模型中，系统能够展现出毫无衰减的完美波函数与密度复苏；然而，一旦在该系统的局域注入一比特量子信息，该信息会迅速在整个系统中发生不可逆的全局加扰，不存在任何形式的信息回流。这一“致命”的反例深刻地揭示了：宏观密度复苏仅仅是信息回流的必要不充分条件。因此，要真正确证 PXP 受限动力学对量子信息的保护机制，引入包含高阶算符演化的测量手段已不再是“锦上添花”，而是拨开迷雾、触及物理本质的唯一合法途径。

为了打开这一多体纠缠黑盒，本章引入了量子信息学中的两个核心高级指标。首先是非时序关联函数（out-of-time-ordered correlator, OTOC）。需要特别澄清的是，尽管 OTOC 在黑洞物理与量子混沌领域通常被用作诊断系统遍历性与提取李雅普诺夫指数（Lyapunov exponent）的工具\cite{hayden2007black,shenker2014black,lewis2019dynamics}，但在本文的受限多体系统中，OTOC 的物理使命发生了根本性转变：它被用作一台“多体显微镜”，精确捕捉算符支撑集（Operator support）在受限空间中独特的周期性膨胀与收缩动力学。其次，鉴于 OTOC 的蝴蝶算符在疤痕态演化的特定相位区间会面临微扰失效的“盲区”，本章进一步引入了基于全状态层析的 Holevo 信息\cite{yuan2022quantum}。作为一种不受限于特定微扰算符的信息论度量，Holevo 信息能够在 OTOC 失去分辨力的时间窗口内，提供对量子信息输运与非马尔可夫（Non-Markovian）回流过程的连续、无死角追踪。

本章的结构安排如下：第 6.2 节将详细介绍在实验上极具挑战的多体时间反演协议，并展示基于 OTOC 的信息塌缩与复苏时空图景；第 6.3 节引入单址分辨的受限量子态层析技术，通过 Holevo 信息与迹距离（Trace distance）对系统的非马尔可夫性进行定量表征；第 6.4 节将深入探究物理机制，通过与上述“玩具模型”的详尽对比，从理论上确证信息复苏是 PXP 受限动力学所特有的唯象后果；最后，第 6.5 节对本章进行总结。
...
(1) 初态制备；(2) 在 $\hat{H}$ 下正向演化时间 $t$；(3) 施加局域微扰与全局翻转；(4) 反向演化时间 $t$； (5) 态布居数探测。
在第 (3) 步中，为了实现完美的时间反演，我们在实验中施加了一个持续约 $\SI{180}{\ns}$ 的远失谐微波脉冲（Microwave pulse），通过交流斯塔克效应在所有格点的里德堡态 $\ket{\uparrow}$ 上诱导全局 $\pi$ 相移。

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.9\textwidth]{chapters/chapter_06/figure/ch6_fig_Sigma_Z.png}
  \caption{\textbf{标定局域微扰以及数值模拟局域微扰保真度对 ZZ-OTOC 测量的影响。}}
  \label{ch6:fig:SigmaZ}
\end{figure}

% \todo{(此处插入图 \ref{ch6:fig:SigmaZ}：对应原 `SI\_OTOC\_details.png` 的图 b，以及原 `SI\_ZZ-OTOC\_Sigma\_Z.png`，用于强调微扰校准的重要性)*)}

同时，对于中心比特的局域微扰 $\hat{W}_c = \sigma_c^z$，我们通过 795 nm 寻址激光诱导约 $2\pi\times\SI{5}{\MHz}$ 的光频移，在约 $\SI{110}{\ns}$ 内实现独立的 $\pi$ 相移。如图 \ref{ch6:fig:SigmaZ} 的单比特 Ramsey 干涉实验结果所示，被寻址中心原子（蓝线）的相位相比未寻址原子（红线）发生了完美的 $1.01(2)\pi$ 偏移。

值得强调的是，局部微扰（蝴蝶算符）的操作保真度对 OTOC 测量的成功与否具有决定性影响。 如数值模拟图 \ref{ch6:fig:SigmaZ}b-e（对应原 `SI\_ZZ-OTOC\_Sigma\_Z.png`）所示，局部微扰累积相位的微小不确定性（例如偏离 $\pi$），会直接导致光锥内 OTOC 塌缩-复苏对比度的衰减，且此类局部操作误差无法被后续的全局误差缓解方案所消除。因此，我们在实验中对该局域相移进行的极致校准，是成功观测加扰动力学的关键前提。
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.8\textwidth]{chapters/chapter_06/figure/SI_ZZ-OTOC_Z2_eff.png}
\caption{\textbf{数值模拟不同的$\ket{\mathbb{Z}_2}$ 态制备保真度对 ZZ-OTOC 动力学的影响。}}
\label{ch6:fig:theory_Z2_Fidelity_OTOC}
\end{figure}

\begin{figure}[!htb]
\centering
\includegraphics[width=0.9\textwidth]{chapters/chapter_06/figure/SI_ZZ-OTOC_exp_Z2.png}
\caption{\textbf{实验$\ket{\mathbb{Z}_2}$ 态制备结果与理想 $\ket{\mathbb{Z}_2}$ 态制备结果对 ZZ-OTOC 动力学的影响。}}
...
如图 \ref{ch6:fig:theory_Z2_Fidelity_OTOC} 所示的模拟表明，若初态保真度大幅降低，远离中心的量子比特上的塌缩-复苏特征将发生严重退化。然而，当我们把实验真实测得的微观态组合（MMC，其中 86\% 为单比特 $\ket{\uparrow} \rightarrow \ket{\downarrow}$ 翻转，总保真度为 78\%）代入含噪模拟时，所得的 OTOC 动力学曲线与完美 $\ket{\mathbb{Z}_2}$ 初态的演化轨迹几乎完全重合（图 \ref{ch6:fig:Z2_Fidelity_70}，对应原 `SI\_ZZ-OTOC\_exp\_Z2.png`）。得益于我们确定的以单比特翻转为主的误差通道，当前的态制备性能已经满足了高保真度 OTOC 观测的需求，初态误差不会对动力学特征产生实质性污染。
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=1.0\textwidth]{chapters/chapter_06/figure/ch6_fig_OTOC_Mitigate.png}
    \caption{\textbf{ZZ/IZ-OTOC测量协议以及ZZ-OTOC 误差缓解方案的数值评估。}}
  \label{ch6:fig:OTOC-Mitigate}
\end{figure}


% \todo{(此处插入图 \ref{ch6:fig:OTOC-Mitigate}：对应原 `SI\_Theory\_Mitigate.png` 以及原 `OTOC.png` 的 d, e 图，展示 IZ/ZZ 协议对比及数值验证)}

\newpage

为了从这些无关的耗散机制中剥离出纯粹的加扰动力学，我们采用了基于 IZ-OTOC 的误差缓解（Error Mitigation）方案。如图 \ref{ch6:fig:OTOC-Mitigate} 所示，我们测量了无微扰的 IZ-OTOC 动力学（即将 $\hat{W}$ 替换为恒等算符 $\hat{I}$）。由于 IZ-OTOC 与 ZZ-OTOC 经历了完全相同的态制备、环境噪声退相干与探测损耗通道，它们共享了同一“衰减包络”。因此，通过计算归一化比值 $F^{c}\approx F^{m}(W,V)/F^{m}(I,V)$，我们可以有效地消除绝大部分相干噪声与残余长程相互作用（$V_{i,i+2}$）的干扰。我们利用蒙特卡罗（Monte Carlo）含噪哈密顿量模拟对该方案进行了严格验证（图 \ref{ch6:fig:OTOC-Mitigate}c 与 \ref{ch6:fig:OTOC-Mitigate}e-f），证明了该方案在恢复由于系统缺陷而模糊的塌缩-复苏特征上具有极高的可靠性。
...
\begin{figure}[!htb]
\centering
\includegraphics[width=1.0\textwidth]{chapters/chapter_06/figure/Z2&trival_OTOC.png}
\caption{\textbf{$\ket{\mathbb{Z}_2}$ 态和$\ket{\mathbf{0}}$态的 ZZ-OTOC、IZ-OTOC 实验结果,误差缓解后的ZZ-OTOC以及数值模拟。}}
\label{ch6:fig:Z2&trival-OTOC}
\end{figure}

% \todo{(此处插入图 \ref{ch6:fig:Z2&trival-OTOC}：对应原 `OTOC.png` 的 f-m 图，展示宏大的时空热图对比)}

缓解后的 OTOC 时空演化热图（图 \ref{ch6:fig:Z2&trival-OTOC}）揭示了这两类初态截然不同的信息输运行为。对于混沌高能初态 $\ket{\mathbf{0}}$（图 \ref{ch6:fig:Z2&trival-OTOC}e），其 OTOC 在局域微扰传播形成的线性光锥（Light cone）内部迅速衰减至零且无任何复苏迹象。这符合强相互作用下的遍历系统（Ergodic systems）预期：局域量子信息被迅速且不可逆地加扰（Scrambled）到了整个庞大的多体希尔伯特空间中。

形成强烈对比的是，$\ket{\mathbb{Z}_2}$ 态表现出了超越传统热化范式的独特动力学（图 \ref{ch6:fig:Z2&trival-OTOC}a）。首先，其“蝴蝶算符传播速度（Butterfly speed）”显著慢于 $\ket{\mathbf{0}}$ 态。并且在清晰的线性光锥边界内，$\ket{\mathbb{Z}_2}$ 态的量子信息可分辨性并没有单调衰减，而是呈现出周期性的塌缩与复苏（Collapse and Revival）图案。实验数据（图 \ref{ch6:fig:Z2&trival-OTOC}c）与无噪理想里德堡哈密顿量下的数值模拟（图 \ref{ch6:fig:Z2&trival-OTOC}d）达到了一致性。

为了更直观、定量地验证这一违背 ETH 的现象，我们将误差缓解后的实验数据与结合了蒙特卡罗方法的含噪数值模拟进行了逐格点（Site-by-site）的细致对比（图 \ref{ch6:fig:mitigated-otoc}）。 如图中含时演化曲线所示，从中心受微扰比特到光锥边缘比特，其测得的复苏振幅、相位延迟以及缓慢的衰减包络，均与包含了真实局域微扰不完美（Local perturbation imperfections）的里德堡/PXP 含噪模拟达到了极高的一致性。这种定量级别的数据吻合，彻底排除了假象的可能，毫无争议地证明了在 PXP 受限多体体系中，动力学约束允许局部扰动以近似弹道的形式传播，并在光锥内部发生周期性的信息回流。


\begin{figure}[!htb]
\centering
\includegraphics[width=0.9\textwidth]{chapters/chapter_06/figure/Extended_Fig_Mitigated_exp_data.png}
\caption{\textbf{分原子展示$\ket{\mathbb{Z}_2}$ 态的缓解 ZZ-OTOC 结果。}}
\label{ch6:fig:mitigated-otoc}
\end{figure}

% \todo{(此处插入图 \ref{ch6:fig:mitigated-otoc}：对应原 `Extende\_Fig\_Mitigated\_exp\_data.png`，展示逐格点的严密对比)}
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=1.0\textwidth]{chapters/chapter_06/figure/SI_fake_collapse.png}
  \caption{
\textbf{无效局域微扰的图示。}}
...
正如我们在图 \ref{ch6:fig:si-fake-collapse} 的热图中观察到的，在某些特定的时间区间（例如 $t \approx 0.6\ \mu\text{s}$ 与 $t \approx 1.2\ \mu\text{s}$），所有量子比特的 ZZ-OTOC 值都异常地逼近于 1（如图 \ref{ch6:fig:si-fake-collapse}b 的虚线标注处）。这是否意味着量子信息在此时刻完美地“重聚”了？答案是否定的。这一现象的物理本质在于：在这些时间节点，$\ket{\mathbb{Z}_2}$ 疤痕态的波函数发生部分复苏，导致中心受微扰量子比特以极大概率处于近乎纯粹的 $\ket{\uparrow}$ 或 $\ket{\downarrow}$ 态。此时，沿 Z 方向的蝴蝶算符 $\hat{W}=\sigma_c^z$ 作用在 $\sigma^z$ 的本征态上，仅仅产生一个全局相位，而无法对系统造成实质性的微扰。由于微扰“失效”，系统演化退化为无微扰的 IZ-OTOC，从而在归一化后产生了一个无物理意义的“假性塌缩（Fake collapse）”。
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=1.0\textwidth]{chapters/chapter_06/figure/HI_sequence_detail.png}
  \caption{
\textbf{Holevo 信息测量的脉冲序列与测量细节。}
}
  \label{ch6:fig:hi-sequence}
\end{figure}

% \todo{(此处插入图 \ref{ch6:fig:hi-sequence}：对应原 `SI\_HI\_sequence.png` 与 `SI\_HI\_dissipative.png`，展示 EIT 屏蔽时序与拉比振荡对比)}

为了在 PXP 受限空间内强行撕开一个独立的单比特操控窗口，我们使用了一种结合全局拉曼驱动与局域 480 nm 电磁诱导透明（EIT）屏蔽的全量子态层析技术（测量时序见图 \ref{ch6:fig:hi-sequence}a）。在演化结束时，我们首先使用经 SLM 调制的局域 480 nm 寻址光照射目标原子的四个近邻与次近邻格点。该光束与 $\ket{e} \rightarrow \ket{r}$ 跃迁共振，通过短暂的耗散过程（约 $\SI{200}{\nano\second}$），将其上可能存在的里德堡布居迅速通过中间态 $\ket{e}$ 自发辐射“清洗”回基态（如图 \ref{ch6:fig:hi-sequence}b 所示，邻近里德堡布居下降超 96\%）。

紧接着，保持这四束 480 nm 寻址光开启。此时，强烈的 480 nm 驱动（拉比频率 $\Omega_{480} \sim 2\pi \times \SI{20}{\MHz}$）与全局 780 nm 激发光形成 EIT 条件。裸里德堡态 $\ket{r}$ 被劈裂为两个相距较远的缀饰态 $\ket{+}$ 和 $\ket{-}$。由于量子干涉效应，这四个相邻原子对基态-里德堡跃迁呈现出完全的透明（禁止跃迁）。在此完美的“光学屏蔽”下，我们施加全局拉曼驱动，此时整个阵列中只有目标原子能够发生高保真度的局域自旋旋转（图 \ref{ch6:fig:hi-sequence}c）。通过扫描脉冲相位测量完整的宇称振荡曲线并提取振幅 $A$，我们能够克服演化间隙残余相互作用带来的相位累积，高精度地重构出 $\rho(t) = \frac{1}{2}(\mathbb{I} + (2P(\uparrow) - 1)\sigma^z) + \epsilon(t)A\sigma^y$。
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.7\textwidth]{chapters/chapter_06/figure/Holevo&Trace.png}
  \caption{\textbf{Holevo 信息探测结果、数值结果和迹距离测量结果。}}
  \label{ch6:fig:holevo&trace}
\end{figure}
...
% \todo{(此处插入图 \ref{ch6:fig:holevo&trace}：结合原 `Holevo.png` 和 `SI\_trace\_distance.png`，展示 Holevo 热图与迹距离)}

如图 \ref{ch6:fig:holevo&trace}a 所示，实验测得的 Holevo 动力学清晰地再现了与 OTOC 一致的线性光锥结构，证实了运动学约束下有限的信息传播速度。更为关键的是，由于 Holevo 信息对所有时刻保持敏感，热图完美地展示了光锥内部连续、清晰的量子信息塌缩与复苏（Collapse and Revival）图案。跟图\ref{ch6:fig:holevo&trace}b 的数值模拟相比，传播模式符合较好。在这段受限演化中，由中心缺陷编码的局域量子信息不仅没有像在遍历系统中那样弥散到整个多体空间中，反而在光锥的特定边界处发生了周期性的汇聚与重现。
...
为了定量坐实这一结论，我们利用重构的密度矩阵计算了随时间演化的迹距离（Trace distance, $D(\rho_1,\rho_2) = \frac{1}{2}\mathrm{Tr}{|\rho_1-\rho_2|}$），它是衡量量子态可分辨性和非马尔可夫性的黄金标准指标。如图 \ref{ch6:fig:holevo&trace}c 所示，迹距离在演化过程中呈现出显著的非单调、周期性增长。这种态可分辨性的周期性上升提供了量子信息回流（Information backflow）的铁证，表明：在 PXP 模型的受限多体框架下，相互作用环境不仅没有彻底抹除局域信息，反而充当了一个相干的“信息回音壁”，使得局域微扰得以以近似弹道的方式传播并周期性地回流。
...
为了在缺乏直接误差缓解方案的情况下验证数据的可靠性，我们采用了正向注入噪声的蒙特卡罗交叉验证策略。我们将 6.2 节中经过严格标定的所有实验不完美因素（初态制备误差、有限温度效应、激光高频噪声等）直接注入到 PXP 哈密顿量的蒙特卡罗数值演化中。如图 \ref{ch6:fig:mitigated-holevo} 所示，仅经过探测误差校正的实验数据（蓝点）与包含了全部演化噪声的模拟结果（实线）展现出了一致性。这种“自下而上”的理论与实验的双向奔赴，不仅印证了我们对系统耗散机制的理解，更确证了光锥内非马尔可夫信息回流这一物理现象的真实性。



\section{波函数复苏与信息回流的非等价性}
