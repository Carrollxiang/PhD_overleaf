...
在一般多体系统中实验测量 OTOC 面临的终极挑战是：如何实现多体相互作用哈密顿量的时间反演（即在 $-\hat{H}$ 下演化）。在此，我们利用了第五章所证明的 PXP 模型的粒子-空穴对称性 $\mathcal{C}=\prod_i \sigma_i^z$。由于 $\mathcal{C}\hat{H}_\text{PXP}\mathcal{C}=-\hat{H}_\text{PXP}$，我们可以通过对所有量子比特施加全局的 $\sigma^z$ 翻转门，将原本的正向演化等效转换为逆向演化。

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.9\textwidth]{chapters/chapter_06/figure/SI_OTOC_sequence.png}
  \caption{OTOC 测量的脉冲序列。}
...
\begin{figure}[!htb]
  ...
  \caption{标定局域微扰以及数值模拟局域微扰保真度对 ZZ-OTOC 测量的影响。}
...
\caption{数值模拟不同的$\ket{\mathbb{Z}_2}$ 态制备保真度对 ZZ-OTOC 动力学的影响。}
...
\caption{实验$\ket{\mathbb{Z}_2}$ 态制备结果与理想 $\ket{\mathbb{Z}_2}$ 态制备结果对 ZZ-OTOC 动力学的影响。}
...
\begin{figure}[!htb]
  ...
  \includegraphics[width=1.0\textwidth]{chapters/chapter_06/figure/ch6_fig_OTOC_Mitigate.png}
    \caption{ZZ/IZ-OTOC测量协议以及ZZ-OTOC 误差缓解方案的数值评估。}
...
\caption{$\ket{\mathbb{Z}_2}$ 态和$\ket{\mathbf{0}}$态的 ZZ-OTOC、IZ-OTOC 实验结果,误差缓解后的ZZ-OTOC以及数值模拟。}
...
\caption{分原子展示$\ket{\mathbb{Z}_2}$ 态的缓解 ZZ-OTOC 结果。}
...
\begin{figure}[!htb]
  ...
  \caption{Holevo 信息探测结果、数值结果和迹距离测量结果。}
...
\begin{figure}[!htb]
  ...
  \caption{初态制备保真度对 Holevo 信息动力学的影响。}
...
\caption{Holevo 信息动力学的实验数据与数值模拟对比。}
...
\begin{figure}[!htb]
  ...
  \caption{玩具模型和 PXP 模型之间疤痕态量子信息动力学的比较。}
  \label{ch6:fig:toy}
\end{figure}
